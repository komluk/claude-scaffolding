...
1. **Markdown SKILL.md** -- guidelines, rules, checklists (shipped in
   `claude-home` at `skills/ui-ux-pro-max/SKILL.md`).
...
- The CSV database is large (thousands of rows) and mutates independently
  from the markdown guidelines. Embedding it in every clone of `claude-home`
...
The markdown `SKILL.md` in `claude-home` has a "Tool Availability" section
